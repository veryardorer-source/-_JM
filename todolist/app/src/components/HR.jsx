...
function lookupTaxTable(monthlyTaxable) {
  ...
  if (monthlyTaxable < 10000000) {
    const idx = Math.min(Math.floor((monthlyTaxable - 3000000) / 20000), TAX_20K.length - 1)
    return TAX_20K[idx]
  }
  // 10,000,000원 이상: 마지막 구간 증가율로 외삽
  const lastTax = TAX_20K[TAX_20K.length - 1]
  const prevTax = TAX_20K[TAX_20K.length - 2]
